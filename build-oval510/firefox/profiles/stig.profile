description: 'This profile is developed under the DoD consensus model and DISA FSO
    Vendor STIG process,
    ...
    and the official DISA FSO STIG baseline, users should expect variance between
    ...
    While this profile is packaged by Red Hat as part of the SCAP Security Guide package,
    please note

    that commercial support of this SCAP content is NOT available. This profile is
    provided as example

    SCAP content with no endorsement for suitability or production readiness. Support
    for this

    profile is provided by the upstream SCAP Security Guide community on a best-effort
    basis. The
