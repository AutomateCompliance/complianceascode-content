...
# complexity = low
# disruption = medium
sysctl_net_ipv6_conf_default_forwarding_value="(bash-populate sysctl_net_ipv6_conf_default_forwarding_value)"
...
if LC_ALL=C grep -q -m 1 -i -e "^net.ipv6.conf.default.forwarding\\>" "/etc/sysctl.conf"; then
    "${sed_command[@]}" "s/^net.ipv6.conf.default.forwarding\\>.*/$formatted_output/gi" "/etc/sysctl.conf"
else
