...
sysctl_net_ipv4_tcp_invalid_ratelimit_value="(bash-populate sysctl_net_ipv4_tcp_invalid_ratelimit_value)"
...
if LC_ALL=C grep -q -m 1 -i -e "^net.ipv4.tcp_invalid_ratelimit\\>" "/etc/sysctl.conf"; then
    "${sed_command[@]}" "s/^net.ipv4.tcp_invalid_ratelimit\\>.*/$formatted_output/gi" "/etc/sysctl.conf"
else
